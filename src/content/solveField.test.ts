...
import { nextScales, readAllScales, readScales, writeScales } from "./solveField.js";
...
describe("fieldScale solver", () => {
  it("reads a scale for every fighter in the roster", () => {
    // Across both files: the cast is split into the owner's private stock
    // photographs and the ten that ship, and a solver that read only one of
    // them would leave the other half unsolved without a word.
    const scales = readAllScales();
